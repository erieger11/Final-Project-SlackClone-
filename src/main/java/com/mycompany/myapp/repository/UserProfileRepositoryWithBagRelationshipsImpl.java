...
public class UserProfileRepositoryWithBagRelationshipsImpl implements UserProfileRepositoryWithBagRelationships {
    ...
    public Optional<UserProfile> fetchBagRelationships(Optional<UserProfile> userProfile) {
        return userProfile.map(this::fetchWorkspaces).map(this::fetchChannels);
    }
    ...
    UserProfile fetchWorkspaces(UserProfile result) {
        return entityManager
            .createQuery(
                "select userProfile from UserProfile userProfile left join fetch userProfile.workspaces where userProfile.id = :id",
                UserProfile.class
            )
    ...
    List<UserProfile> fetchWorkspaces(List<UserProfile> userProfiles) {
        ...
        List<UserProfile> result = entityManager
            .createQuery(
                "select userProfile from UserProfile userProfile left join fetch userProfile.workspaces where userProfile in :userProfiles",
                UserProfile.class
            )
