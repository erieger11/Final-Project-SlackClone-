...
function DirectMessages({ currentUser }) {
  ...
  const [userRef] = useState(MySQL.database().ref('users'))
  const [connectedRef] = useState(MySQL.database().ref('.info/connected'))
  const [presenceRef] = useState(MySQL.database().ref('presence'))
